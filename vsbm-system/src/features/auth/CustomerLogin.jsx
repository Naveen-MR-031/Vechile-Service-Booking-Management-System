...
import { Mail, Lock, ArrowRight, Car, Shield, Clock, Star, KeyRound, Loader2, ArrowLeft } from 'lucide-react';
...
const CustomerLogin = () => {
    ...
    // Resend OTP (re-validates password)
    ...
    const handleVerifyOTP = async (e) => {
        ...
        try {
            ...
            toast.success(`Welcome back, ${result.first_name || result.company_name}!`);
            ...
            <motion.div className={styles.visualSide} initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
                <div className={styles.visualContent}>
                    ...
                    <p className={styles.visualDescription}>
                        Premium vehicle maintenance at your fingertips. Track your service in real-time, get transparent pricing, and enjoy hassle-free booking.
                    </p>
                    <div className={styles.features}>
                        <div className={styles.feature}><Clock size={20} /><span>Real-time Service Tracking</span></div>
                        <div className={styles.feature}><Shield size={20} /><span>Verified Service Centers</span></div>
                        <div className={styles.feature}><Star size={20} /><span>Rated & Trusted Providers</span></div>
                    </div>
                </div>
            ...
            <motion.div className={styles.formSide} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
                <div className={styles.formContainer}>
                        ...
                        {step === 'credentials' && (
                            <motion.div key="cred-step" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                                <div className={styles.formHeader}>
                                    ...
                                    <p className={styles.formSubtitle}>Sign in to manage your bookings</p>
                                </div>
                    ...
                    <p className={styles.signupPrompt}>
                        New user? <Link to="/signup" className={styles.signupLink}>Create an account</Link>
                    </p>
                    <p className={styles.providerPrompt}>
                        Service provider? <Link to="/provider/login" className={styles.signupLink}>Partner Login</Link>
                    </p>
                </div>
            </motion.div>
        </div>
